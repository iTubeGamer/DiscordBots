			...
			for (TempChannelMap tempChannelMap : tempChannelsByGuild.values()) {
				...
				for (TempChannel tempChannel : tempChannelMap.getAllTempChannel()) {
					...
					if (!tempChannel.getChannel().isDeleted()) {
						...
						Logger.warn("Guild:\"{}\" | Event:DeletedChannelException | Channel:\"{}\"", tempChannel.getChannel().getGuild(), tempChannel.getChannel().getName());
					}
				}
			}
	...
	private void updateTempChannelAndSendNotifications(List<TempChannel> tempChannelToDelete, TempChannel tempChannel) {
		if (tempChannel.getChannel().getConnectedUsers().isEmpty()) {
			...
			if (tempChannel.getEmptyMinutes() > tempChannel.getTimeoutInMinutes()) {
				Logger.info("Guild:\"{}\" | Event:ChannelDeletion | Channel:\"{}\" | Reason:Timeout({})",
						tempChannel.getChannel().getGuild().getName(), tempChannel.getChannel().getName(), tempChannel.getTimeoutInMinutes());
				tempChannelToDelete.add(tempChannel);
				sendPrivateMessage(tempChannel.getOwner(), "Your TempChannel `" + tempChannel.getChannel().getName() + "` got deleted.");
			} else if (tempChannel.getTimeoutInMinutes() < 10 && (tempChannel.getTimeoutInMinutes() - tempChannel.getEmptyMinutes()) == 0) {
				sendPrivateMessage(tempChannel.getOwner(),
						"Your tempChannel `" + tempChannel.getChannel().getName() + "`  will be **deleted in a minute** if you do not use it.");
			} else if ((tempChannel.getTimeoutInMinutes() - tempChannel.getEmptyMinutes()) == 4 && tempChannel.getTimeoutInMinutes() >= 10) {
				sendPrivateMessage(tempChannel.getOwner(),
						"Your tempChannel `" + tempChannel.getChannel().getName() + "` will be **deleted in 5 minutes** if you do not use it.");
			}
		} else if (tempChannel.getEmptyMinutes() > 0) {
			tempChannel.setEmptyMinutes(0);
			Logger.warn("Guild:\"{}\" | Event:InvalidEmptyMinutes | Channel:\"{}\"", tempChannel.getChannel().getGuild(), tempChannel.getChannel().getName());
		}
	}
